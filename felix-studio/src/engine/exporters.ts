...
/**
 * Every export carries a manifest naming the token/content fingerprint it was
 * generated from. The studio records the same fingerprint, and the audit
 * flags the export as stale when the product moves past it — maintenance
 * that reaches the shipped artifact, not just the canvas.
 */
export function manifestFor(state: StudioState, screen: Screen) {
  return {
    generator: 'felix-studio',
    schema: 1,
    screen: screen.name,
    surface: screen.surface,
    business: state.business.name,
    fingerprint: exportHash(screen, state.tokens, state.business),
  };
}
...
<!-- felix-manifest ${JSON.stringify(manifestFor(state, screen))} -->
<html lang="en">
<head>
<meta charset="utf-8">
...
function exportChatFlow(state: StudioState, screen: Screen): ExportArtifact {
  const flow = {
    ...
    _felix_manifest: manifestFor(state, screen),
    version: '1.0',
    business: {
